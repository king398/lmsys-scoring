...
from transformers import AutoTokenizer, LlamaForSequenceClassification, TrainingArguments, Trainer, DataCollatorWithPadding, AutoModelForCausalLM
...
def main():
    ...
    # delete where target==2
    train = train.loc[train['target']!=2].reset_index(drop=True)
    def make_chosen_prompt(df):
        if (df['target']==0):
            prompt = df['prompt'] + "\n\n" +  df['response_a']
        elif df['target']==1:
            prompt = df['prompt'] + "\n\n" + df['response_b']
    ...
    train['chosen'] = train.apply(lambda x: make_chosen_prompt(x), axis=1)
    train['rejected'] = train.apply(lambda x: make_rejected_prompt(x), axis=1)
    ...
    train_df.drop(['text', 'text_len', 'target', 'fold','model_a','model_b','id','response_a','response_b','winner_model_a','winner_model_b','winner_tie'], axis=1, inplace=True)
    valid_df.drop(['text', 'text_len', 'target', 'fold','model_a','model_b','id','response_a','response_b','winner_model_a','winner_model_b','winner_tie'], axis=1, inplace=True)
    ...
    #tokenizer_name = "Kukedlc/NeuralLLaMa-3-8b-ORPO-v0.2"
    ...
    training_args = ORPOConfig(
        ...
        bf16=True,
        output_dir=f"paligemma_rec_lora_kalora_test_fold{args.fold}_dpo",
        push_to_hub=True,
    )
